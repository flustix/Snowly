...
public class CalculateCommand : IOptionSlashCommand {
    ...
    public void Handle(DiscordInteraction interaction) {
        RealmAccess.Run(realm => {
            ...
            var messagesMin = (long)Math.Ceiling((float)xpLeft / rngMax);
            var messagesMax = (long)Math.Ceiling((float)xpLeft / rngMin);
            var messagesAvg = (long)Math.Ceiling(xpLeft / ((rngMin + rngMax) / 2f));
